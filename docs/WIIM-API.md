...
| Purpose | Command |
|---|---|
| Read default bands | `EQGetLV2BandEx:<pluginURI>` |
| Read a source's bands | `EQGetLV2SourceBandEx:<json{source_name,pluginURI}>` |
| Write bands | `EQSetLV2SourceBand:<json{source_name,pluginURI,channelMode,EQBand[]}>` |
| Enable / disable for a source | `EQChangeSourceFX` / `EQSourceOff:<json{source_name,pluginURI}>` |
| List presets | `EQv2GetList:<pluginURI>` |
| Load / save preset | `EQv2SourceLoad` / `EQSourceSave:<json{…,Name}>` |
| Delete / rename preset | `EQv2Delete` / `EQv2Rename:<json{pluginURI,Name[,newName]}>` |

JSON arguments are URL-encoded. The EQ card self-disables if the device stops answering these (firmware kill-switch). Implemented in `src/lib/wiim/eq.ts` + `eq-constants.ts`.
...
### Input names & availability _(community-verified)_

| Purpose | Command |
|---|---|
| Custom input names set in the WiiM app | `getModeRename` → `{ <mode>: <name> }`; replies `Failed` when nothing is renamed |
| Which inputs are enabled in the WiiM app | `getAudioInputEnable` → `{ audioInput: [{ mode, enable }] }` (`enable` 0/1) |

`getModeRename` keys are switchmode-style mode strings (e.g. `line-in`, `optical`, `usbdac`); the app maps them to source keys and uses them as the default input label. Inputs disabled in the WiiM app (`enable: 0`) are hidden from the switcher, except the currently-active one.
...
### Output sound-card modes / USB DAC _(community-verified)_

`getSoundCardModeSupportList` lists the supported output sound-card modes. A connected **USB DAC** appears as mode `AUDIO_OUTPUT_UAC_CARD_MODE` with a `devName` (the DAC's name). Note: these named modes do **not** map to the numeric `setAudioOutputHardwareMode` value above.
